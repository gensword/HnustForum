...
@section('message')
    ...
    <div class="col-md-8 visible-xs-block"  style="background-color:white; border: #e5e5e5 1px solid;min-height: 250px;">
        <div class="row" style="margin-left: 15px; margin-top: 25px;"><h5 ><a href="{{ url('/users/'.Auth::id().'/messages') }}"><span class="iconfont icon-fanhui1" style="font-size: 15px"></span>返回</a></h5></div>
        <div class="row" style="margin-left: 15px">发送私信给 {{ $otherSideUser->username }}</div>
        <div class="row" style="margin-left: 0px;">
            <div class="col-md-8">
        ...
        @foreach($messages as $message)
            <div class="row" style="margin-left: 15px">
                <div class="col-md-1 col-xs-4"><a href="{{ url('/users/'.$message->from_uid) }}"><img src="{{ $message->fromUser->avatar }}" alt="..." class="img-circle img-responsive" > </a></div>
                <div class="col-md-6"><p>
                        @if( \App\Husers::find($message->from_uid)->id == Auth::id() )
                            我
                        @else
    ...
    <script>
        ...
        $('.sendMsg').on('click', function () {
            ...
            $.post("{{ url('/sendMessages') }}", {toUid:toUid, contents:content}, function(data){
                ...
                '<div class="col-md-1 col-xs-3"><a href="{{ url('/users/'.Auth::id()) }}"><img src='+ data.avatar + ' alt="..." class="img-circle img-responsive" > </a></div>'+
